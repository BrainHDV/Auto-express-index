...
function switchToStateFromURLHash() {
  ...
  switch (SPAState.pagename) {
    case "main":
      pageHTML += `
      <div class="main">
        <h3 class="main__title">Энциклопедия пива</h3>
        <p class="main__text">Здесь содержится информация о разных сортах пива.</p>
  ...
  function dataLoaded(data) {
    ...
    const html = `
      <nav>
        <h3 class='content__title'>${SPAState.title}</h3>
        <ul>
          ${sortedData
